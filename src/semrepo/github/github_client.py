...
class AllTokensExhaustedError(RuntimeError):
    """Raised when every configured token is rate-limited on the same call."""
...
class GitHubClient:
    ...
    def get(self, path: str) -> requests.Response:
        ...
        configured token is rate-limited on this call."""
        ...
        while tokens_tried < len(self.tokens):
            ...
            logger.warning(
                "Token %d/%d rate-limited, rotating", self._token_index + 1, len(self.tokens)
            )
            self._token_index = (self._token_index + 1) % len(self.tokens)

        raise AllTokensExhaustedError(f"All {len(self.tokens)} tokens are rate-limited for {url}")